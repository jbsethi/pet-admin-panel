...
const UpdateOrderModal = ({
  ...
  return (
    <CModal size="lg" show={show} onClose={() => setShow(false)}>
      <CModalHeader>Shopping, Grooming and others</CModalHeader>
      <CModalBody>
        <CDataTable
          fields={receiptTableFields.filter(
            (field) => !disableUpdate || field.key !== "actions"
          )}
          items={items || []}
          footer
          hover
          overTableSlot={
            <div className="d-flex justify-content-between align-items-center">
              <div>
                <strong>
                  Checkup Price:{" "}
                  <span className="text-primary">{checkUpPrice || 0} AED</span>
                </strong>
              </div>
              <div>
                <strong>
                  Total Bill :{" "}
                  <span className="text-primary">{total || 0} AED</span>
                </strong>
              </div>
              {!disableUpdate && (
          ...
          scopedSlots={{
            vat: (item) => (
              <td>{item.price * (isVatIncluded ? vatPercentage / 100 : 0)}</td>
            ),
            discount: (item) => <td>{`${item.discount} %`}</td>,
            total: (item) => (
              <td>
                {item.total +
                  (isVatIncluded ? item.total * (vatPercentage / 100) : 0)}
              </td>
            ),
            actions: (item) => (
      ...
      <CModalFooter className="d-flex justify-content-between">
        <div className="d-flex">
          <CButton size="sm" color="info" onClick={handlePrint}>
            Print Invoice
          </CButton>

          <CLabel htmlFor="vat" className="ml-4">
            Apply{" "}
            <span className={styleItems["inline-input-span"]}>
              <input
                style={{
                  width: `${15 + (vatPercentage.length - 1) * 8}px`,
                }}
                className={styleItems["inline-input"]}
                value={vatPercentage}
        ...
        <div>
          ...
          {!disableUpdate && (
            <CButton
              size="sm"
              color="primary"
              onClick={() => handleAction({ type: "updateReceipt" })}
            >
              Update
            </CButton>
          )}
